...
lRate = 0.000000001  #takes 5 iterations
...
def costFunction(i):
  ...
  temp1 = 0
  for n in range(ncols-1):
    regError = 0.5*regConstant*weight[i,n]*weight[i,n]
    sumRegError = temp1 + regError
    temp1 = sumRegError
  costFunc = costFunc + sumRegError 
  return costFunc
...
def weightUpdate(i):
  ...
  for n in range(ncols-1):
    for j in range(nrows-1):
      error = (predValue(j,i)) - (newArray[j,2])
      minCostFuncPI = ((error)*newArray[j,n]) # + regConstant*weight[i,n] (not required as change is accomodated in final weight equation )
      summedError = minCostFuncPI + temp
      temp = summedError      
    weight[i+1,n] = (1-(lRate*regConstant))*weight[i,n]-lRate*(summedError)
